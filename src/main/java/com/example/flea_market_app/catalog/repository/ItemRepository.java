...
public interface ItemRepository extends JpaRepository<ItemEntity, UUID> {
	...
	List<ItemEntity> findByStatusAndCategoryId(@Param("status") String status, @Param("categoryId") UUID categoryId,
			Pageable pageable);

	@Query("SELECT i FROM ItemEntity i WHERE i.status = :status " +
			"AND (LOWER(i.name) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(i.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
	List<ItemEntity> findByStatusAndKeyword(@Param("status") String status, @Param("keyword") String keyword,
			Pageable pageable);
